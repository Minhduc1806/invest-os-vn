...
def main():
    macro=load(LIVE/'macro_rates_live.vn.json'); fund=load(LIVE/'fundamentals_live.vn.json')
    items=[
      ('vnstock_rate_limit_hardening', True, 'run_pipeline --no-refresh + cache-first market refresh + data_adapters throttle/stop'),
      ('fundamentals_full_financials', False, 'Company.overview real; Finance tables empty in free vnstock, full valuation blocked until paid/API source'),
    ...
    out={'as_of':now_iso(),'completion_pct':round(score*100,1),'items':[{'name':n,'done':d,'note':note} for n,d,note in items],'macro_deposit_confidence':macro.get('rates',{}).get('deposit_rates',{}).get('confidence'),'fundamental_status':fund.get('status')}
