...
def main()->int:
    ...
    rates=macro.get('rates',{}); dives=fund.get('deep_dive',[]); news_items=news.get('items',[])[:20]; idx=market.get('indices',[]); positions=portfolio.get('positions',[]); rows=bars_table(ohlcv)
